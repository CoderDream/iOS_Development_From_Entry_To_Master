...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate{
    ...
    func buildUI()
    {
        ...
        button.setTitle("截取图片", for: UIControlState.init(rawValue: 0))
    ...
    func captureOutput(_ captureOutput: AVCaptureOutput!,didOutputSampleBuffer sampleBuffer: CMSampleBuffer!,from connection: AVCaptureConnection!) {
        autoreleasepool {
            ...
            {
                ciImage = ciImage.applying(CGAffineTransform(rotationAngle: CGFloat(M_PI / -2.0)))
            }
            ...
            {
                ciImage = ciImage.applying(CGAffineTransform(rotationAngle: CGFloat(M_PI)))
            }
    ...
    func captureScreen(_ sender: UIButton)
    {
